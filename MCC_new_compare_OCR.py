...
for i, (p_i, m_i) in enumerate(zip(points, markers)):
    plt.plot(p_i * dt, eqp_inc_history[p_i] / dt, f'b{m_i}',ms=10, mfc='none');
...
xmin, xmax = plt.xlim()
plt.plot([-load_length * dt, 2 * load_length * dt], [0, 0], ':b')
plt.xlabel(r'Time $t$ [s]')
plt.ylabel(r'Axial strain rate $\dot{\varepsilon_{a}}$ [1/s]')
ymin, ymax = plt.ylim()
plt.ylim(ymin, ymax + np.ceil(y_offset * 10) / 10)
plt.xlim(xmin, xmax)
plt.savefig(f"load_history.png", dpi=300, bbox_inches="tight")

for pc_0, ls in zip([150, 450], ['-',':']):
    # Declarations
    ...
    # pressure vs. time
    plt.subplot(2, 4, 1)
    plt.plot(np.arange(load_length) * dt, p, ls+'b', label='qstat')
    plt.plot(np.arange(load_length) * dt, p_total, ls+'g', label='tot')
    # begin and endpoints
    ...
    plt.subplot(2, 4, 2)
    plt.plot(np.arange(load_length) * dt, q, ls+'b', label='qstat')
    plt.plot(np.arange(load_length) * dt, q_total, ls+'g', label='tot')
    # begin and endpoints
    ...
    ax1.plot(np.arange(load_length) * dt, eq, ls+'b', label='dev')
    ...
    ax2.plot(np.arange(load_length) * dt, ev, ls+'g', label='v')
    ...
    plt.subplot(2, 4, 4)
    plt.plot(np.arange(load_length) * dt, pc_history, ls+'b')
    # begin and endpoints
    ...
    plt.subplot(2, 4, 5)
    plt.plot(np.arange(load_length) * dt, p_c / p, ls+'b')
    # begin and endpoints
    ...
    plt.subplot(2, 4, 6)
    imark = 500
    plt.plot(p, q, ls+'b', markevery=imark, label='qstat')
    plt.plot(p_total, q_total, ls+'g', label='tot')
    plt.plot([0, np.max(p)], [0, M * np.max(p)], ls+'r', label='CSL')
    # begin and endpoints
    ...
    # void ratios vs. p
    plt.subplot(2, 4, 7)
    plt.plot(p, void_ratio_q, ls+'b', label='qstat')
    plt.plot(p_total, void_ratio_total, ls+'g', label='tot')
    # begin and endpoints
    ...
    plt.plot(np.arange(load_length) * dt, p, ls+'b', label=r"Quasi-static pressure ($p^{\mathrm{q}}$)")
    plt.plot(np.arange(load_length) * dt, p_total, ls+'g', label=r"Total pressure ($p$)")
    ...
    plt.plot(np.arange(load_length) * dt, q, ls+'b', label=r"Quasi-static deviatoric stress ($q^{\mathrm{q}}$)")
    plt.plot(np.arange(load_length) * dt, q_total, ls+'g', label=r"Total deviatoric stress ($q$)")
    ...
    plt.plot(np.arange(load_length-1) * dt, p_c[1:] / p[1:], ls+'b', label=r'Pressures ($p^{\mathrm{c}}/p^{\mathrm{q}}$)')
    plt.plot(0 * dt, p_c[1] / p[1], 'b.', ms=10, label='')
    plt.plot(load_length * dt, p_c[-1] / p[-1], 'bx', ms=10, label='')
    plt.plot(np.arange(load_length-1) * dt, q_c[1:] / q[1:], ls+'g', label=r'Deviatoric stresses ($q^{\mathrm{c}}/q^{\mathrm{q}}$)')
    plt.plot(0 * dt, q_c[1] / q[1], 'g.', ms=10, label='')
    plt.plot(load_length * dt, q_c[-1] / q[-1], 'gx', ms=10, label='')
    for i, (p_i, m_i) in enumerate(zip(points, markers)):
        plt.plot(p_i * dt, p_c[p_i]/p[p_i], f'b{m_i}',ms=10, mfc='none')
        plt.plot(p_i * dt, q_c[p_i]/q[p_i], f'g{m_i}',ms=10, mfc='none')
    plt.xlabel(time_label)
    ...
    plt.plot([0, np.max(p) * 1.5], [0, M * np.max(p) * 1.5], ls+'r', label='Critical state line')
    plt.plot(p, q, ls+'b', markevery=500, label=r"Quasi-static stress")
    plt.plot(p_total, q_total, ls+'g', label=r"Total stress")
    ...
    plt.plot(p, void_ratio_q, ls+'b', label=r"Quasi-static void ratio")
    plt.plot(p_total, void_ratio_total, ls+'g', label='Total void ratio')
    ...
    plt.plot(np.arange(load_length) * dt, q / p, ls+'b', label=r"Quasi-static friction ($\mu^{\mathrm{q}}$)")
    plt.plot(np.arange(load_length) * dt, q_total / p_total, ls+'g', label=r"Total friction ($\mu$)")
    ...
    plt.plot(np.arange(load_length) * dt, void_ratio_q-void_ratio_q[0], ls+'b', label=r"$e^{\mathrm{p}}-e^{\mathrm{p}}_0$")
    plt.plot(np.arange(load_length) * dt, void_ratio_e-void_ratio_e[0], ls+'g', label=r"$e^{\mathrm{e}}$")
    for i, (p_i, m_i) in enumerate(zip(points, markers)):
        plt.plot(p_i * dt, void_ratio_q[p_i] - void_ratio_q[0], f'b{m_i}',ms=10, mfc='none')
        plt.plot(p_i * dt, void_ratio_e[p_i] - void_ratio_e[0], f'g{m_i}',ms=10, mfc='none')
    ...
    plt.plot(eqp_inc_history / dt, (q_total/p_total), ls+'b')
    plt.xlabel(r'Axial strain rate $\dot{\varepsilon_{a}}$ [1/s]')
    plt.ylabel(mu_label)
    plt.ylim(1)
...
p_yield = np.linspace(50,100,100);
...
plt.plot(pCSL, qCSL, 'r',label='Critical state line')
plt.plot(p_yield,q_yield,'k-',label='Yield surface');
plt.plot(p_yield[0],q_yield[0],'ko');
plt.text(p_yield[0], q_yield[0] - 7, r'$(p_1^{\mathrm{q}}, q_1^{\mathrm{q}})$', fontsize=12)  # label near point A

p_yield = np.linspace(25,50,100);
q_yield = (M**2*(50*p_yield-p_yield**2))**0.5;    
plt.plot(p_yield,q_yield,'k-');
plt.plot(p_yield[0],q_yield[0],'ko');
plt.text(p_yield[0], q_yield[0] - 7, r'$(p_2^{\mathrm{q}}, q_2^{\mathrm{q}})$', fontsize=12)  # label near point B

p_yield = np.linspace(37.5,75,100);
